...
sub getIDandLabel() {
	...
	return ($id, $label)
    if (($label eq 'Cause-Effect')       || ($label eq 'Cause-Effect')       ||
		($label eq 'Component-Whole')    || ($label eq 'Component-Whole')    ||
		($label eq 'Content-Container')  || ($label eq 'Content-Container')  ||
		($label eq 'Entity-Destination') || ($label eq 'Entity-Destination') ||
		($label eq 'Entity-Origin')      || ($label eq 'Entity-Origin')      ||
		($label eq 'Instrument-Agency')  || ($label eq 'Instrument-Agency')  ||
		($label eq 'Member-Collection')  || ($label eq 'Member-Collection')  ||
		($label eq 'Message-Topic')      || ($label eq 'Message-Topic')      ||
		($label eq 'Product-Producer')   || ($label eq 'Product-Producer'));
